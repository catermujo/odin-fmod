...
when ODIN_OS == .Windows {
	when ODIN_ARCH == .amd64 {
		foreign import lib "windows_x64/fsbank_vc.lib"
	} else {
		#panic("vendor/fmod fsbank supports windows amd64 only")
	}
} else when ODIN_OS == .Linux {
	when ODIN_ARCH == .amd64 {
		foreign import lib "linux_x64/libfsbank.so"
	} else {
		#panic("vendor/fmod fsbank supports linux amd64 only")
	}
} else when ODIN_OS == .Darwin {
	when ODIN_ARCH == .amd64 {
		foreign import lib "darwin_x64/fsbank.dylib"
	} else when ODIN_ARCH == .arm64 {
		foreign import lib "darwin_arm64/fsbank.dylib"
	} else {
		#panic("vendor/fmod fsbank supports Darwin amd64/arm64 only")
	}
}
